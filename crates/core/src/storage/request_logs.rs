...
impl Storage {
    ...
    pub fn list_request_logs_paginated(
        ...
        let sql = format!(
            "SELECT
                r.trace_id, r.key_id, r.account_id, r.client_ip, r.initial_account_id, r.attempted_account_ids_json, r.initial_aggregate_api_id, r.attempted_aggregate_api_ids_json,
    ...
    /// 函数 `ensure_request_log_client_ip_column`
    ///
    /// 作者: gaohongshun
    ///
    /// 时间: 2026-04-27
    ...
    /// # 返回
    /// 返回函数执行结果
    pub(super) fn ensure_request_log_client_ip_column(&self) -> Result<()> {
        self.ensure_column("request_logs", "client_ip", "TEXT")?;
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_request_logs_client_ip_created_at ON request_logs(client_ip, created_at DESC)",
            [],
        )?;
        Ok(())
    }
...
fn map_request_log_row(row: &Row<'_>) -> Result<RequestLog> {
    Ok(RequestLog {
        ...
        transparent_mode: row.get(14)?,
        enhanced_mode: row.get(15)?,
        model: row.get(16)?,
